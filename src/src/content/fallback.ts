...
/**
 * Generate an offline placeholder image
 */
async function generateOfflinePlaceholder(): Promise<ImageData> {
  const now = Date.now();
 ...
 * Tries to download from Unsplash, falls back to embedded placeholder if offline or all downloads fail
 */
export async function getFallbackImages(): Promise<ImageData[]> {
  ...
  if (validImages.length === 0) {
    console.warn('All fallback downloads failed, generating offline placeholder');
    const placeholder = await generateOfflinePlaceholder();
    return [placeholder];
  }
